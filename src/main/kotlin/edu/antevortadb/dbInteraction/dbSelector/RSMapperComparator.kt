...
object RSMapperComparator: Comparator<RSMapper> {
    ...
    override fun compare(rs1: RSMapper, rs2: RSMapper): Int {
        // columns with order *must* be set before this function can work, it's the only way to know
        // how to sort the mappers, by date, name, or other. If this is null, it wasn't set, leave
        // things unsorted by telling the sortWith function using this comparator that all values
        // are equal and log an error message. The user should reset this variable to null when
    ...
    private fun compareMappers(rs1: RSMapper, rs2: RSMapper): Int {
        // End the recursion, the columns to be used for comparison have been exhausted, declare
        // the objects equal
    ...
    private fun compareAscending(rs1: RSMapper, rs2: RSMapper, columnName: String): Int {
        return when {
            ...
            rs1.getString(columnName) < rs2.getString(columnName) -> -1
            // Values are equal, make the recursive call, compareMappers will end the recursion
            // when there's no more columns
            else -> compareMappers(rs1, rs2)
        }
    }

    private fun compareDescending(rs1: RSMapper, rs2: RSMapper, columnName: String): Int {
        // Just use compareAscending, reverse the return value or make the recursive call if
        // values are equal
    ...
    /*
        If the column doesn't exist, or it's not comparable, getString will return "". In this case
        return true, and the calling function will know to return a no-compare value, 0, and move on
     */
