 ...
 * SelectionWorker passed to the genericSelect function it will query the proper DB files
 * and use the proper RSMapper object
 */
@Suppress("unused", "MemberVisibilityCanBePrivate", "HasPlatformType")
abstract class Selector{
    // dbTableName and listOfColumns get set in the derived class c'tors. dbTableName is
    // used to select the right table from the DB shards (e.g. submission_attrs) for
    // reddit submissions. listOfColumns is used by the orderby function for determining
    // which words in a query refer to a valid column.
...
//----------------------------------------------------------------------------------------
// NOTE: This can be used when one of the functions below doesn't satisfy your querying
// needs. I suggest just writing the function below unless you're absolutely positive the
// query is a one-off so you don't need to keep writing the same SQL string.
//----------------------------------------------------------------------------------------
    abstract fun generalSelection(SQLStatement: String): List<RSMapper>

//----------------------------------------------------------------------------------------
// The following functions take the dbTableName variable (set the in the derived class
// default c'tors) and create an sql query string using the DBSelector class. The query
// string is passed back down to a derived class through generalSelection to ascertain
// which workers to use and which DB shards to search before calling the generalized
...
//----------------------------------------------------------------------------------------
    ...
    fun selectAllWhereColumnEquals(columnName: String, equalsValue: String):
            List<RSMapper> {
        val sel = DBSelector()
                .from(this.tableName)
    ...
    fun selectAllWhereColumnLessThan(columnName: String, lessThanValue: String):
            List<RSMapper> {
        val sel = DBSelector()
                .from(this.tableName)
    ...
    fun selectAllWhereColumnGreaterThan(columnName: String, greaterThanValue: String):
            List<RSMapper> {
        val sel = DBSelector()
                .from(this.tableName)
    ...
    fun selectAllAfterDate(year: Int, month: Int, day: Int, hour: Int,
        ...
        val utc =
                TimeUtils.utcSecondsFromValues_SEL(year, month, day, hour, minute, second)
    ...
    fun selectAllBetweenDates(startYear: Int, startMonth: Int, startDay: Int,
            ...
            List<RSMapper> {
        val start = LocalDateTime.of(startYear, startMonth, startDay,
                startHour, startMinute, startSecond)
        val end = LocalDateTime.of(endYear, endMonth, endDay, endHour,
                endMinute, endSecond)
...
//----------------------------------------------------------------------------------------
// The functions below implement the queries on the DB. verifyDBsExist simply checks that
// the requests DB shards are in place on the disk where they're expected to be. If this
// fails the program dies...something is very wrong if this failed. genericSelect is a
// generalized DB query function which gives a list of selection workers will search the
// proper DB shards.
//----------------------------------------------------------------------------------------
    ...
    protected fun genericSelect(workers: List<SelectionWorker>, SQLStatement: String):
            List<RSMapper> {
        ...
        workers.mapTo(futureResults) {
            executor.submit(it) as Future<ArrayList<RSMapper>>
        }

        // Get all results from the worker threads and place the RSMappers in the
        // results ArrayList
        ...
        // The research machine has multiple DB shards which makes "orderby" requests
        // almost useless. The code below will search for orderby in the query, if it
        // exists it will determine which column name the ordering is to be done on and
        // if the ordering is ascending or descending (default assumes ascending). After
        // checking for the order by command and sorting (if necessary) we check for a
        // limit command. If there is a limit command return only the requested number of
        // results, in sorted order (by order by, or created_dt if no order by)
    ...
    /*
        Takes in the query and the results, checks the query to see if it contains an
        orderby clause, if it does it will process the orderby, determine how to sort the
        results based on the columns the ordering should be done on and if the ordering
        should be ascending (default) or descending, then it will return the properly
        sorted results. If there is no orderby clause the results will be returned
        unmodified.
     */
    private fun handleOrderBy(query: String, results: List<RSMapper>): List<RSMapper>{
        // If the query doesn't contain "orderby" and it doesn't contain "order by", or
        // the query returned no results return the results without further processing
        ...
        // Determine which columns names should be sorted, and if it should be sorted
    ...
    /*
        Determine which column should be used in the sorting when an order by clause is
        added to the SQL query It takes in the query string and returns a map of column
        names and a boolean. The boolean is true when the sorting should be done in
        ascending order (the default) and false when it should be done in descending
        order.
     */
    private fun determineOrderByColumns(query: String): OrderBySelection {
        val theOrdering = OrderBySelection()
        // Split the string on the order by command, the left side doesn't matter, right
        // side has the order by info
        ...
        for(columnAndOrder in columnsWithOrdering) {
            ...
            // See if the column exists in the the table list, if it doesn't get out
            // of here
    ...
    /*
        ...
        If isAscending is true the data should be sorted in ascending order, if it's
        false it should be sorted in descending order
     */
    private fun doTheSort(results: List<RSMapper>, columnsWithOrder: OrderBySelection):
            List<RSMapper> {
        val mutableResults = results.toMutableList()

        // Set the OrderBySelection before doing the sort. Unfortunately this can't be
        // done in the Comparator function call because the comparator implements a
        // specific interface. Oh well.
    ...
    /*
        When the limit command is given it is given to all 6 shards, resulting in 6x as
        many results as one actually wants to receive. This function limits the results
        as if they're all coming from a single DB, as that's what the user expects. If
        there was an order by command the results this function receives will already be
        sorted as the user wants, if there is no order by command this function will
        default to a sort based on created_dt, the creation time of the element in the DB
        as this is how the data is stored in the DB (for the most part). This will,
        however, perform a full created_dt sort as there are some data inconsistencies
        around this for stored elements in the DBs
     */
    private fun handleLimit(SQLQuery: String, results: List<RSMapper>): List<RSMapper> {
        ...
        // NOTE: This is >= so that we can just return the results if the number of
        // expected results is equal to the number of results we have. If this was just
        // ">" (as one would expect) we would unnecessarily  copy the results without
        // ignoring any of the returned results
    ...
    private fun getLimitNumber(SQLStatement: String): Int {
        // Splitting on limit guarantees the first word in the second array element will
        // be the number of results the user expects (assuming a proper SQLStatement)
        val splitOnLimit = SQLStatement.split("limit")

        // Splitting it on spaces guarantees that the first elements in the split will be
        // the limit number after cleaning up any extra leading or trailing spaces. This
        // works when the limit command is the last command as well as when it's buried
        // in a string with a bunch of other commands
        val limitNumberStringSplits = splitOnLimit[1].trim().split(" ")

        // Get the limit number and perform a final clean. There shouldn't be any
        // non-printing characters on the string now but clean it up all the same
    ...
    /*
        Get selector type based on string matching...stupid idea, will revisit another
        time NOTE: The companion object is kotlin specific
     */
    companion object {
        fun getSelectorOnType(input: String): Selector {
            return when {
                input.toLowerCase()
                        .contains(Finals.REDDIT_COM_TABLE) -> RedditComSelector()
                input.toLowerCase()
                        .contains(Finals.REDDIT_SUB_TABLE) -> RedditSubSelector()
                input.toLowerCase()
                        .contains(Finals.ML_GENOME_TAGS_TABLE) -> MLGenomeTagsSelector()
                input.toLowerCase()
                        .contains(Finals.ML_INDIVIDUAL_RATING_TABLE) ->
                ...
                input.toLowerCase()
                        .contains(Finals.TMDB_CREDITS_TABLE) -> TMDBCreditsSelector()
                else -> throw IllegalArgumentException(
                        "Selector.getSelectorOnType is a stupid idea, Sean")
            }
        }
    }
}
